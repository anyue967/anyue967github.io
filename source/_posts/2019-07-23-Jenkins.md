...
#### 安装Jenkins 192.168.37.10
[Installing Jenkins](https://www.jenkins.io/doc/book/installing/)
```bash
# apache-tomcat(java -jar jenkins.war)
...
# /etc/hosts
# 当Jenkinsi访问updates.jenkins-ci.org,就会被Nginx转到国内
...
pipeline {
    ...
    stages {
        stage('1. 环境检查') {
            steps {
                echo '===== 正在检查内置节点基础环境 ====='
                // 输出当前运行 Jenkins 的系统用户，确保不是 root 用户在运行
                sh 'echo "当前运行用户: $(whoami)"'
                // 输出系统内核和版本信息
                sh 'uname -a'
            }
        }
        ...
        stage('2. 验证上一轮配置') {
            steps {
                echo '===== 正在验证之前配置的 Jenkins URL 环境变量 ====='
                // 验证您之前在 Jenkins Location 中配置的 Jenkins 根 URL 是否生效
                echo "当前的 Jenkins 根 URL 是: ${env.JENKINS_URL}"
                echo "当前这次构建的具体 URL 是: ${env.BUILD_URL}"
            }
        }
        ...
        stage('3. 模拟工具链测试') {
            steps {
                ...
                // 检查系统自带的开发工具是否可用
                sh 'java -version'
                sh 'git --version || echo "提示：未安装 Git"'
            }
        }
    }
    ...
    post {
        success {
            echo '🎉 测试成功！内置节点工作正常，可以安全地进行本地测试。'
        }
        failure {
            echo '❌ 测试失败！请检查控制台日志中的具体报错原因。'
        }
    }
}
```
