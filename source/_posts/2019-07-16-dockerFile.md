...
- [DockerFile 语法](#dockerfile-语法)
  - [docker\_practice](#docker_practice)
  - [运行](#运行)
  - [FROM -- 指定基础image](#from----指定基础image)
  - [MAINTAINER --指定镜像创建者信息](#maintainer---指定镜像创建者信息)
  - [RUN --安装软件用](#run---安装软件用)
  - [CMD --设置 container 启动时执行的操作(覆盖)](#cmd---设置-container-启动时执行的操作覆盖)
  - [ENTRYPOINT --设置 container 启动执行的操作(追加)](#entrypoint---设置-container-启动执行的操作追加)
  - [USER --设置 container 用户](#user---设置-container-用户)
  - [EXPOSE --指定容器需要映射到宿主机的端口](#expose---指定容器需要映射到宿主机的端口)
  - [ENV --用于设置环境变量](#env---用于设置环境变量)
  - [ADD --从 src 复制文件到 container 的 dest 路径(解包后拷贝)](#add---从-src-复制文件到-container-的-dest-路径解包后拷贝)
  - [COPY --从 src 复制文件到 container 的 dest 路径](#copy---从-src-复制文件到-container-的-dest-路径)
- [VOLUME --指定挂载点](#volume---指定挂载点)
  - [WORKDIR --切换目录](#workdir---切换目录)
  - [ONBUILD --在子镜像中执行](#onbuild---在子镜像中执行)
  - [构建示例(构建PHP)](#构建示例构建php)
- [构建示例(构建JAVA)](#构建示例构建java)
  - [构建示例(tomcat)](#构建示例tomcat)
...
#### CMD --设置 <span style="color:rgb(255, 0, 255);">container 启动时执行的操作(覆盖)</span>
+ 设置指令，用于 container 启动时指定的操作，该操可以是自定义的脚本，也可以是执行系统的命令，该指令只能在文件中存在一次，若有多个，只执行最后一条
  ```Dockerfile
  CMD echo "Hello，World！"
  ```

#### ENTRYPOINT --设置 <span style="color:rgb(255, 0, 255);">container 启动执行的操作(追加)</span>
...
#### ADD --从 src 复制文件到 container 的 dest 路径<span style="color:rgb(255, 0, 255);">(解包后拷贝)</span>
+ ADD [src] [dest]
...
| MAINTAINER | 镜像维护者信息                                                       | ENTRYPOINT  | <span style="color:rgb(255, 0, 255);">运行容器执行的shell命令</span> |
| RUN        | <span style="color:rgb(255, 0, 255);">构建镜像运行的shell命令</span> | VOLUME      | 指定容器挂载点到宿主机自动生成的目录                                 |
| CMD        | <span style="color:rgb(255, 0, 255);">运行容器执行的shell命令</span> | USER        | 为RUN/CMD/ENTRYPOINT执行命令指定用户                                 |
