...
- [Linux 集群](#linux-集群)
  - [1.liunx 集群概述：](#1liunx-集群概述)
    - [1.1 集群概念：](#11-集群概念)
    - [1.2 集群(Cluster)的分类：](#12-集群cluster的分类)
  - [2.负载均衡集群(LBC)](#2负载均衡集群lbc)
    - [2.1LVS 相关原理：](#21lvs-相关原理)
    - [2.2LVS工作模式：](#22lvs工作模式)
  - [3.集群通用算法](#3集群通用算法)
    - [3.1静态调度算法：](#31静态调度算法)
    - [3.2动态调度算法：](#32动态调度算法)
    - [3.3持久化连接(类似于SH优先级最高)](#33持久化连接类似于sh优先级最高)
    - [3.4ipvsadm编译安装](#34ipvsadm编译安装)
  - [4.LVS工作模式构建：](#4lvs工作模式构建)
    - [4.1 NAT 模式构建：](#41-nat-模式构建)
    - [4.2负载调度器配置(VIP,DIP):](#42负载调度器配置vipdip)
    - [4.3真实服务器配置:](#43真实服务器配置)
    - [4.4DR 模式构建：](#44dr-模式构建)
    - [4.5负载调度器配置](#45负载调度器配置)
    - [4.6真实服务器](#46真实服务器)
  - [5.高可用集群(HAC)](#5高可用集群hac)
    - [5.1Keeplived原理：](#51keeplived原理)
    - [5.2LVS-DR+Keeplived：](#52lvs-drkeeplived)
    - [5.3负载调度器 1](#53负载调度器-1)
    - [5.4负载调度器 2](#54负载调度器-2)
    - [5.5真实服务器](#55真实服务器)
    - [5.6Heartbeat+Nginx：](#56heartbeatnginx)
...
+ NAT(多目标的DNAT) ✔
  - 模式特点：
    ...
    * 集群节点，必须在一个网络中；
    * 真实服务器网关必须指向负载调度器；
    * RIP：Real Server IP通常都是私有IP，仅用于各个集群节点通信；
